...
function apllyCheckEvents () {
	...
	$("form.user_check").on("change", "input:checkbox", function() {
		...
		if(this.checked == true){
			...
			//////// 	ajax con bd para headhunters contables	//////////////
			if(area == "headhunter"){
				$.ajax({
				type: "POST",
				url: "funciones.php",
				data: {hh:item_id, tipo:'add'},
			...
			//////// 	ajax con bd para reclutadores  contables	//////////////
			if(area == "recruiter"){
				$.ajax({
				type: "POST",
			...
			if(highlight.length){
				...
				//////// 	ajax con bd para headhunters contables (remove)	//////////////
				if(area == "headhunter"){
				$.ajax({
				type: "POST",
				url: "funciones.php",
				data: {hh:item_id, tipo:'remove'},
				...
				//////// 	ajax con bd para reclutadores  contables (remove)	//////////////
				if(area == "recruiter"){
				$.ajax({
				type: "POST",
				url: "reclutadores.php",
				...
				//////// 	ajax con bd para jobs  contables (remove)	//////////////
				if(area == "job"){
					$.ajax({
					type: "POST",
					url: "jobs-cont.php",
...
$('form.ajax').on('submit', function(event) {
	...
	.fail(function() {
		swal({
			title: _('Error'),
			html: _('Desculpenos. Hubo un error al registrar el puesto'),
			type: 'error',
		}).then(function() {
			// OK
		}, function(dismiss) {
...
$('form.ajax1').on('submit', function(event) {
	event.preventDefault();
	var $form = $(this);
	$form.find('[type="submit"]').prop('disabled', true);
	blockUI();
	$.ajax({
		url: 'registroTrabajo.php',
		type: 'POST',
		data: $form.serialize(),
	})
	.done(function(data) {
		...
		//comentando alerta de localhost:
		//alert(data);
		
		// NÃ£o houve retorno, ou retornou erro
		if (! data || ! data.success) {
			$('#modal-found-a-job').modal('hide');
			$form.get(0).reset();
			swal({
				title: _('Exito'),
				html: data,
...
///////////////////////////
// FORMULARIO DESEAS MEJORAR COMO EMPRESA//
///////////////////////////
...
$(document).ready(function(){
	$('form.ajax2').submit(function (e){
		...
		if($date == 'exito'){
			$('#contacto').modal('hide');
			swal('Envio Exitoso','Enviado','success');
		}else{
			$('#contacto').modal('hide');
...
function paginacion(numero,pagina,area){
	$.ajax({
		type:"POST",
		url:pagina,
		data:"pagina="+numero,
	...
	.fail(function() {
			$(area).empty().append('<p>Error</p>');
	})
}

function paginacionjobs(numero,pagina,area){
	var busqueda = $("#jobsSearch").val();
	var accion = 1;
	$.ajax({
		type:"POST",
		url:pagina,
		data:"pagina="+numero+"&busqueda="+busqueda+"&accion="+accion,
		//dataType: 'json',
	})
	.done(function(datos){
		$(area).empty().append(datos);
...
function paginationUltimo(ruta2,numero){
	$.ajax({
		type:"POST",
		url:"mostrarRespuesta.php",
		data:"idforo="+ruta2+"&pagina="+numero,
		//dataType: 'json',
		
	})
	.done(function(datos){
		$('#message-list').empty().append(datos);

		if (datos.respuesta != null) {
			$('#message-list').append(datos);
		}
	...
	.fail(function() {
			$('#message-list').empty().append('<p>Error</p>');
			$('#panel-collapse-threads').collapse('hide');
			$('#panel-collapse-messages').collapse('show');
			$('#message-list').load();
	})
...
$('#forum-wrapper').on('click', '.list-group-item-heading a', function(event) {
	event.preventDefault();
	licenciaid = $(this).attr('class');
	id = $(this).attr('id');
	nombre =$(this).attr('name');
	ruta = id;
	numero ="1";

	paginationUltimo(ruta,numero);
...
/* Inicio Análisis Empleabilidad */
$('#employability-tab').on('submit', '#AnalisisEmp', function(event) {
	event.preventDefault();
	var $form = $(this);
	$.ajax({
		url			: $form.attr('action'),
		...
		success: function(response){
		swal(response);
		document.getElementById("AnalisisEmp").reset();
	}
	})
});

/* Fin Análisis Empleabilidad */
...
$('#forum-wrapper').on('submit', '#forum-post-form, .forum-reply-form', function(event) {
	...
	$.ajax({
		...
		data		: $form.serialize()+'&paginas='+numero,
	})
	.done(function(data) {
		
		swal('Mensaje Registrado');
		$('#message-list').empty().append(data);
		//document.getElementById("Respuestas_foro").value=respuesta_foro;
		document.getElementById('Respuestas_foro').innerHTML=respuesta_foro;
		document.getElementById(id_rpta).innerHTML=rptas_foro;
		...
		/*var message = _('Desculpame. Houve um erro ao registrar sua mensagem.');
	...
	.fail(function() {
		var message = _('Desculpe Hubo un problema al registrar su mensaje');
		swal(_('Error'), message, 'error');
	});
});
...
if ($myFileUploadForm.length) {
	$myFileUploadForm.on('submit', function(event) {
		...
		$myFileUploadForm.ajaxSubmit({
			dataType: 'json',
			url: $myFileUploadForm.prop('action'),//problema
			success: function (data) {
				var message = _('Desculpe. Houve um erro ao enviar seu arquivo.');
				message = data && data.message && data.message.length ? data.message : message;
				if (! data || ! data.success) {
					swal(_('Erros'), message, 'error');
					return;
				}
				swal({
					'title'	:_('Sucesso'),
...
if ($empregForm.length) {
	...
	$empregForm.validate({
		...
		invalidHandler: function (e, validator) {
			var message = '<p>' + _('hay¡ problemas de validación en formuraio. Verifique los campos');
			if (validator.errorList && validator.errorList.length) {
				message += ':</p>';
				message += '<ul class="list-unstyled">';
				$(validator.errorList).each(function () {
...
/////////////////
// Envio do arquivo
$('#cv-upload form').on('submit', function(event) {
	event.preventDefault();
	//var $form = $(this);
	if($('div.checkbox-group.required :checkbox:checked').length > 0) {
		var parametros = new FormData($("#traduccion_cv")[0]);
		$.ajax({
		data: parametros,
		url:"traduccion.php",
		type: "POST",
		...
		success: function(response){
		swal(response);
		document.getElementById("traduccion_cv").reset();
	}
		});
	}else{
	swal(_('Error'), _('Seleccione al menos un lenguaje para traducir su CV'), 'error');
}
});
...
function openTrainingArea() {
	...
	$.ajax({
		url:'Entrenamiento1.php',
		success:function(data){
			$('.contenido-entrenamiento').html(data);
...
function openLearning() {
    $('#big-areas').addClass('hide');
    $('#fases-learning').removeClass("hide");
}
var c=0;
...
function openNavArea(tabName = null) {
	...
	if(tabName=='manual') {
		newTitle = _('Manual de Usuario');
		newDescription = '';
	}
	else if(tabName=='fases-genes') {
...
function backToHome(e) {
	...
	$('#fases-genes').addClass('hide');
	$('#areas-genes').addClass('hide');
	$('#fases-learning').removeClass("show");
	$('#fases-learning').addClass('hide');
	$('#itens-menu').addClass('hide');
...
$('.foro').click(function(){
	...
	$.ajax({
		type: "post",
		url: "mostrarRespuesta.php",
		data: {mensaje:'visitas', id:id},
		success: function (respuesta) {
			$('.visitas'+id).text(respuesta);
		},
		fail: function(){
			alert('ERROR');
		}
...
//Llamada al actualizar perfil ---------------------------------------

$('#perfilUsuario1 form').on('submit', function(event) {
	
	event.preventDefault();
	if($('div.CheckReq.required :checkbox:checked').length > 0) {
		var parametros = new FormData($("#perfilUsuario2")[0]);
		 $.ajax({
			data: parametros,
			...
			success: function(response){
				...
				if($respuesta == 'error1'){
					swal({
    				title: 'Error',
    				text: 'Suba un archivo de imagen.',
    				type: 'error',
    				});
				...
				$('#perfilUsuario1').append(response);
				document.getElementById('nom_img').value=nom_imagen;
				var img=document.getElementById('nom_img').value;
				if(img === ''){
					$(".previsualizar").attr("src","image/users/usuario_default.png");
					$(".previsualizar1").attr("src","image/users/usuario_default.png");	
				}else{
					$(".previsualizar").attr("src",img);
					$(".previsualizar1").attr("src",img);	
				}
			}	
		}
			});
	}else{
		swal(_('Error'), _('Es necesaria su autorización'), 'error');
	}
});
//--------------------------------------------------------------------
//validar sesion
$("#iniciarSesion").on('submit',function(e){
	...
	$.ajax({
		...
		success: function (respuesta) {
			...
			if(respuesta=="expirada")
			{
				$('#userError2').removeClass("hidden")
			}
		},
...
$("#archivo").change(function (e) { 
	...
	if (archive["type"] != "application/pdf") {
		$("#archivo").val("");
		swal('Error', "Solo puedes subir Archivos PDF", 'error');
	}
});

// Envio do arquivo
document.addEventListener("DOMContentLoaded", () => {
	let form = document.getElementById('my-file-upload-form');
...
function subir_archivo(form){ 
	...
	peticion.open('post', 'misArchivos.php');
	peticion.onreadystatechange = function(){

		if(this.readyState == 4 && this.status == 200){
			if (this.responseText == "guardado") {
				$("#archivo").val("");
				$("#my-file-upload-title").val("");
				swal('Ok', "Se guardo correctamente tu Archivo", 'success');
				ListarMisArchivos();
			}
			if (this.responseText == "no-pago") {
				$("#archivo").val("");
				$("#my-file-upload-title").val("");
				swal('Error', "Lo invitamos a que se suscriba para contar con estos beneficios", 'error');
			}
		}
	}
...
$("#misArchivos").on('click',function(e){
	e.preventDefault();
	$("#ArchivosMi").slideToggle();
})
...
function ListarMisArchivos() {
	$.ajax({
		...
		success: function (response) {
			...
			var archiv = "";
			json.forEach(json => {
				if (json.titulo_archivo == "") {
					archiv = "ArchivoPDF";
				}else{
					archiv = json.titulo_archivo;
				}
				html += `<div class="col-4 col-md-3 col-lg-2">
					<div style="border:1px solid #e12b2c; border-radius:2px;padding: 5px;text-align: center;">
						<i class="far fa-file-pdf"></i>
						<a target="_blank" href="${json.ruta_archivo}">${archiv}</a>
					</div>
				</div>
				`;
			});
			$("#ArchivosMi").html(html);
		}
	});
}
...
$("#linkedin-file-upload-input").change(function (e) { 
	e.preventDefault();
	var archive = this.files[0];
	var nombre = archive["name"].split(".")[1];
	if (archive["type"] != "application/x-zip-compressed" && archive["type"] != "application/vnd.ms-excel" && nombre != "rar") {
		$("#linkedin-file-upload-input").val("");
		$(".text-file").val("");
		swal('Error', "Solo puedes subir Archivos ZIP o CSV", 'error');
	}
});

document.addEventListener("DOMContentLoaded", () => {
	let form = document.getElementById('linkedin-file-upload-form');
	form.addEventListener("submit", function(event){
		event.preventDefault();
		subir_archivoL(this);
	});
...
$("#abrir_linke").on('click',function(e){
	e.preventDefault();
	$("#miLinke").slideToggle();
})


function subir_archivoL(form){ 
	let peticion = new XMLHttpRequest()
	peticion.open('post', 'miLinkedin.php');
	peticion.onreadystatechange = function(){
...
function ListarMisLinkedinActivo(page, dato) {
	$.ajax({
		type: "POST",
		url: "miLinkedin.php",
		data: "listarM="+page+"&dato="+dato,
		success: function (response) {
...
function ListarMisLinkedinNoActivo(page, dato) {
	$.ajax({
		type: "POST",
		url: "miLinkedin.php",
		data: "listaVa="+page+"&dato="+dato,
		success: function (response) {
			console.log(response);
			$("#contactado").html(response)									
		}
	});
}

ListarMisLinkedinActivo();
ListarMisLinkedinNoActivo();

	$(document).on("click", ".pages-linked", function (e) {
		e.preventDefault();
		var page = $(this).attr("page");console.log("nav");
		ListarMisLinkedinActivo(page);
	});

	$(document).on("click", ".pages-linked2", function (e) {
		e.preventDefault();
		var page = $(this).attr("page");console.log("nav");
		ListarMisLinkedinNoActivo(page);
	});

	$(document).on("click",".check-ajax", function () {
		var idLink = $(this).parent().parent().find(".input-check").val();
		if( $(this).prop('checked') ) {
			$.ajax({
				type: "POST",
				url: "miLinkedin.php",
				data: "actuaL=2&id="+idLink,
				beforeSend:function () { 
					$("#linkedin-load").removeAttr("style");
					$("#disponible").html("");
					$("#linkedin-checked-load").removeAttr("style");
					$("#contactado").html("");
				},
				success: function (response) {
					$("#linkedin-load").attr("style", "display: none;");
					$("#linkedin-checked-load").attr("style", "display: none;");
					ListarMisLinkedinActivo();
					ListarMisLinkedinNoActivo();
				}
			});
...
$("#formLinke").submit(function (e) { 
	e.preventDefault();
	var dato = $("#linkedinSearch").val();
	ListarMisLinkedinNoActivo("",dato);
	ListarMisLinkedinActivo("",dato);
});

$("#quitar-filL").click(function (e) { 
	e.preventDefault();
	ListarMisLinkedinNoActivo();
	ListarMisLinkedinActivo();
	$("#linkedinSearch").val("");
...
$(document).on("click", ".pages-proceso", function (e) {
	e.preventDefault();
	var page = $(this).attr("page");console.log("nav");
	ListarProceso(page);
});
...
$("#mi_proceso").submit(function (e) { 
	e.preventDefault();
	const proceso = $(this).serialize();
	$.ajax({
		type: "POST",
		url: "miProceso.php",
		data: proceso,
		success: function (response) {
			if (response == "guardado") {
				$('#mi_proceso')[0].reset();
				swal('Ok', "Se guardo correctamente sus datos", 'success');
				ListarProceso()
			}
			if (response == "no-pago") {
				$('#mi_proceso')[0].reset();
...
$(document).on("click",".input_bt", function () {
	var valorB = $(this).parent().parent().find(".proceso").val();
	$("#proceso-i checked-progress").removeClass("checked-progress");
	$.ajax({
		type: "POST",
		url: "miProceso.php",
		data: "detalle_pro="+valorB,
		success: function (response) {
			console.log(response);
			$(this).parent().parent().find(".panel-heading").addClass("checked-progress");
			$("#proceso-v").html(response);
		}
	});
})

$("#form_caza").submit(function (e) { 
	e.preventDefault();
	
});

document.addEventListener("DOMContentLoaded", () => {
	let form = document.getElementById('form_caza');
	form.addEventListener("submit", function(event){
		event.preventDefault();
		let peticions = new XMLHttpRequest()
		peticions.open('post', 'cazaProceso.php');
		peticions.onreadystatechange = function(){
	
			if(this.readyState == 4 && this.status == 200){
				if (this.responseText == "guardado") {
					swal('Ok', "Se Registro su Contacto Correctamente", 'success');
					form.reset();
					ListarCaza();
				}
				else if (this.responseText == "no-pago") {
					swal('Error', "Lo invitamos a que se suscriba para contar con estos beneficios", 'error');
				}
				else{
		...
		peticions.send(new FormData(form));
	});
});

function ListarCaza(page) {
	$.ajax({
		type: "POST",
		url: "cazaProceso.php",
		data: "listarC="+page,
...
$(document).on("click", ".pages-caza", function (e) {
	e.preventDefault();
	var page = $(this).attr("page");console.log("nav");
	ListarCaza(page);
});

$(document).on("click",".input_ca", function () {
	var valorB = $(this).parent().parent().find(".caza").val();
	$("#proceso-i checked-progress").removeClass("checked-progress");
	$.ajax({
		type: "POST",
		url: "cazaProceso.php",
		data: "detalle_ca="+valorB,
		success: function (response) {
			$(this).parent().parent().find(".panel-heading").addClass("checked-progress");
			$("#caza_detalle").html(response);
		}
	});
})

function ListarCazaRecibo(page) {
	$.ajax({
		type: "POST",
		url: "cazaProceso.php",
		data: "listarR="+page,
		success: function (response) {
			console.log(response);	
			$("#indication-template").html(response)							
		}
	});
}
...
$(document).on("click",".input_caR", function () {
	var valorB = $(this).parent().parent().find(".caza").val();
	$("#proceso-i checked-progress").removeClass("checked-progress");
	$.ajax({
		type: "POST",
		url: "cazaProceso.php",
		data: "detalle_caR="+valorB,
		success: function (response) {
			$(this).parent().parent().find(".panel-heading").addClass("checked-progress");
			$("#indication-templateD").html(response);
		}
	});
})