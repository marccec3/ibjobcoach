 ...
 if(!isset($_SESSION['usuario'])){
   ...
   $iniciar = ($paginaActual-1)* $cantidadMostrar;
    if(isset($_POST['mensaje'])){
        ...
        $queryConsulta = $connect->prepare("SELECT vistas FROM foro WHERE idforo = ?");
        $queryConsulta->execute([$_POST['id']]);
        $queryVisitas = $queryConsulta->fetch(PDO::FETCH_OBJ);

        $nuevo_num_vistas= $queryVisitas->vistas+1;

        $querysuma = $connect->prepare("UPDATE foro SET vistas = ? where idforo=?");
        $querysuma->execute([$nuevo_num_vistas,$_POST['id']] );
        echo $nuevo_num_vistas;
        ...
        */
        ...
        <span class="date"><i class="fas fa-fw fa-clock-o text-muted"></i>'.$respuestas->fecha.'</span>
        </p>
        </div>
        <div class="col-md-6">
        <div>
